...
{  //Creating a socket with (domain IPv4, TCP communication,
                           // IP protocol)
   //socket_fd is the socket descriptor (an integer).
   socket_fd = ::socket (AF_INET, SOCK_STREAM, 0);
   if (socket_fd < 0) throw socket_sys_error ("socket");
   int on = 1;
   //setsockopt- used to prevent errors such as: "address already
   //in use".
...
//Invoked after the creation of the socket. Binds the socket to the
//address and port number specified in port.
...
//Puts the server socket in a passive mode, waits for the client
...

//The first connection request is extracted from the queue of
//pending requests. At this point, connection is established
//between client and server and data is ready to be transferred.
