...
//Class: listmap
//key_t, mapped_t, less_t are generic data types.
...
class listmap 
{
   ...
   private:
      less_t less;  //Determines the ordering of the list (lexicographic)
      struct node;
      struct link   //Taken care of by node struct.
      {
         node* next{};
         node* prev{};
      ...
      //Anchor casts a link to a node.
      //Anchor->next returns the head of the list and 
      //Anchor->prev returns the tail.
      node* anchor() { return static_cast<node*> (&anchor_); }
      //Returns the next and previous nodes of anchor.
